...
class OrderMigrator extends OrderMigration {
    ...
    public function get_dokan_order_data( $parent_order_id, $seller_id ) {
        ...
        $prepared_sql = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}wcfm_marketplace_orders orders WHERE orders.vendor_id = '%d' AND orders.order_id = '%d'", $seller_id, $parent_order_id );
        ...
        foreach ( $orders as $key => $order ) {
            ...
            $meta_sql  = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}wcfm_marketplace_orders_meta order_meta WHERE order_meta.order_commission_id = '%d'", $order->ID );
    ...
    public function process_refund( $child_order, $seller_id, $from_suborder = true ) {
        global $wpdb;
        $refund_id = ''; $status = 'completed'; $refund_note = '';
        ...
        $sql .= " AND vendor_id = %d";
        $sql .= " AND order_id = %d";
        $refund_infos = $wpdb->get_results( $wpdb->prepare( $sql, $seller_id, $this->order_id ) );
